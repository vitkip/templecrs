<div>
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-headline-sm font-bold text-on-surface">ໝວດເອກະສານ / Document Categories</h1>
                <p class="text-body-sm text-on-surface-variant mt-0.5">ຈັດການໝວດໝູ່ເອກະສານທັງໝົດ / Manage all document categories</p>
            </div>
        </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
        ...
        <div class="bg-surface-container rounded-xl border border-outline-variant p-4 flex items-center gap-3 col-span-2 sm:col-span-1">
            ...
            <div>
                <p class="text-[10px] font-bold text-on-surface-variant uppercase tracking-wide">ເອກະສານທັງໝົດ</p>
                <p class="text-title-md font-bold text-on-surface">{{ $categories->sum('doc_count') }}</p>
            </div>
        </div>
    </div>
    ...
    @if ($categories->isEmpty())
        <div class="bg-surface-container rounded-2xl border border-outline-variant py-16 flex flex-col items-center gap-3 text-center">
            ...
            <p class="text-body-md text-on-surface-variant">ຍັງບໍ່ມີໝວດເອກະສານ / No categories yet</p>
            <button wire:click="create"
        ...
        <div class="bg-surface-container rounded-2xl border border-outline-variant overflow-hidden">
            <div class="divide-y divide-outline-variant">
                @foreach ($categories as $cat)
                    @php $colorClass = \App\Models\DocumentCategory::$colorMap[$cat->color] ?? 'text-gray-700 bg-gray-50 border-gray-200'; @endphp
                    <div class="flex items-center gap-4 px-4 py-3 hover:bg-surface-container-high/40 transition-colors group">
                        ...
                        <div class="text-center hidden sm:block shrink-0">
                            <p class="text-title-sm font-bold text-on-surface">{{ $cat->doc_count }}</p>
                            <p class="text-[10px] text-on-surface-variant">ເອກະສານ</p>
                        </div>
            ...
            <div class="bg-surface rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto"
                ...
                <div class="flex items-center justify-between px-6 py-4 border-b border-outline-variant sticky top-0 bg-surface z-10">
                    <h2 class="text-title-md font-bold text-on-surface">
                        {{ $editingId ? 'ແກ້ໄຂໝວດເອກະສານ / Edit Category' : 'ເພີ່ມໝວດໝູ່ໃໝ່ / New Category' }}
                    </h2>
                ...
                <form wire:submit="save" class="px-6 py-5 space-y-4">
                    ...
                    <div>
                        ...
                        <input wire:model.live="name_lo" type="text"
                               placeholder="ຕ.ວ. ຄຳສັ່ງ, ແຈ້ງການ..."
                    ...
                    <div>
                        ...
                        <input wire:model="name_en" type="text"
                               placeholder="e.g. Order, Announcement..."
                               class="w-full px-3 py-2.5 bg-surface-container border border-outline-variant rounded-xl text-body-md focus:outline-none focus:ring-2 focus:ring-primary/20" />
                    </div>
                    ...
                    <div>
                        ...
                        <input wire:model="slug" type="text"
                               placeholder="order, announcement, report..."
                               {{ $editingId ? 'readonly' : '' }}
                        ...
                        <p class="mt-1 text-[11px] text-on-surface-variant">ຕົວພິມນ້ອຍ, ຕົວເລກ, - ຫຼື _ ເທົ່ານັ້ນ · ໃຊ້ເກັບໃນຖານຂໍ້ມູນ</p>
                    ...
                    <div>
                        ...
                        <div class="flex flex-wrap gap-2">
                            ...
                            @endphp
                            @foreach (\App\Models\DocumentCategory::$colorOptions as $c)
                                <button type="button" wire:click="$set('color', '{{ $c }}')"
                                        title="{{ $c }}"
                                        class="w-8 h-8 rounded-full {{ $colorDots[$c] ?? 'bg-gray-400' }} transition-all
                    ...
                    {{-- Preview --}}
                    @php $prevColor = \App\Models\DocumentCategory::$colorMap[$color] ?? 'text-gray-700 bg-gray-50 border-gray-200'; @endphp
        ...
        <div class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <div class="bg-surface rounded-2xl p-6 max-w-sm w-full shadow-xl">
                ...
                <p class="text-body-md text-on-surface-variant mb-6">
                    ທ່ານແນ່ໃຈບໍ່ທີ່ຈະລຶບໝວດນີ້? ການກະທຳນີ້ບໍ່ສາມາດຍ້ອນຄືນໄດ້.
                </p>
