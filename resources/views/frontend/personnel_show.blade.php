...
@keyframes lotus-pulse {
    0%,100% { box-shadow: 0 0 0 3px #C8953A, 0 0 0 8px rgba(200,149,58,0.22), 0 6px 36px rgba(200,149,58,0.14); }
    50%      { box-shadow: 0 0 0 3px #C8953A, 0 0 0 16px rgba(200,149,58,0.08), 0 6px 48px rgba(200,149,58,0.07); }
}
@keyframes fade-up {
    from { opacity:0; transform:translateY(18px); }
    to   { opacity:1; transform:translateY(0); }
}
.ps-reveal { animation: fade-up 0.55s ease both; }
.ps-reveal-1 { animation-delay: 0.05s; }
.ps-reveal-2 { animation-delay: 0.13s; }
.ps-reveal-3 { animation-delay: 0.21s; }
.ps-reveal-4 { animation-delay: 0.29s; }
.ps-reveal-5 { animation-delay: 0.37s; }
@media (prefers-reduced-motion: reduce) {
    .ps-reveal { animation: none !important; }
}
</style>
...
<div class="relative overflow-hidden"
    ...
    <div class="relative z-10 max-w-5xl mx-auto px-4 sm:px-8 pt-8 pb-16">
        ...
        <nav class="flex items-center gap-2 mb-10" aria-label="Breadcrumb">
            ...
            <span style="color:rgba(255,255,255,0.18); font-size:11px;">›</span>
            <span style="font-size:11px; color:rgba(255,255,255,0.65);" class="truncate max-w-[180px]">{{ $name }}</span>
        </nav>
            ...
            <div class="mb-6 shrink-0">
                @if ($photoUrl)
                    <img src="{{ $photoUrl }}" alt="{{ $name }}" loading="eager"
                        class="w-40 h-40 sm:w-48 sm:h-48 rounded-full object-cover"
                        style="{{ $isMonk
...
<div style="background-color:#FAF6EF;">
    ...
    <div class="flex items-center justify-center py-5" aria-hidden="true">
        <div style="height:1px; width:80px; background:linear-gradient(to right, transparent, rgba(200,149,58,0.3));"></div>
        <svg class="mx-4 shrink-0" width="24" height="24" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div style="height:1px; width:80px; background:linear-gradient(to left, transparent, rgba(200,149,58,0.3));"></div>
    </div>
        ...
        <div class="lg:flex lg:gap-12">
            ...
            <div class="flex-1 min-w-0 space-y-8">
                        ...
                        <div class="flex"
                            ...
                            @if ($pansa)
                                <div class="flex-1 px-6 py-5 text-center"
                                    ...
                                    <p class="font-bold tabular-nums" style="font-size:36px; line-height:1; color:#C8953A;">{{ $pansa }}</p>
                                    <p class="font-bold uppercase tracking-widest mt-1.5"
                                        style="font-size:9px; color:rgba(124,77,15,0.5); letter-spacing:0.16em;">ພຣັ​ສສາ</p>
                                </div>
                            @endif
                            @if ($ordinationDate)
                                <div class="flex-1 px-6 py-5 text-center">
                                    <p class="font-bold" style="font-size:20px; line-height:1.2; color:#7C4D0F;">{{ $ordinationDate }}</p>
                                    <p class="font-bold uppercase tracking-widest mt-1.5"
                ...
                @if ($bio)
                    <section class="ps-reveal ps-reveal-2">
                        <div class="flex items-center gap-3 mb-4">
                            <span class="material-symbols-outlined shrink-0" style="font-size:16px; color:#C8953A;">auto_stories</span>
                            <h2 class="font-bold tracking-widest uppercase"
                                style="font-size:11px; color:#3D2A12; letter-spacing:0.14em;">ຊີວະປະຫວັດ</h2>
                            <div class="flex-1 h-px" style="background:linear-gradient(to right, rgba(200,149,58,0.28), transparent);"></div>
                        </div>
                ...
                @if ($education)
                    <section class="ps-reveal ps-reveal-3">
                        <div class="flex items-center gap-3 mb-4">
                            <span class="material-symbols-outlined shrink-0" style="font-size:16px; color:#7C4D0F;">school</span>
                            <h2 class="font-bold tracking-widest uppercase"
                                style="font-size:11px; color:#3D2A12; letter-spacing:0.14em;">ລະດັບການສຶກສາ</h2>
                            <div class="flex-1 h-px" style="background:linear-gradient(to right, rgba(124,77,15,0.22), transparent);"></div>
                        </div>
                        <div class="rounded-2xl px-5 py-5 sm:px-6" style="background:white; border:1px solid rgba(0,0,0,0.06);">
                ...
                @if (!$bio && !$education && !$currentTemple)
                    <div class="text-center py-16">
                        <span class="material-symbols-outlined" style="font-size:44px; color:rgba(200,149,58,0.2);">person</span>
            ...
            <aside class="w-full lg:w-60 shrink-0 mt-10 lg:mt-0 space-y-4">
                ...
                @if ($person->phone || $person->email || $person->facebook)
                    <div class="rounded-2xl p-4 ps-reveal ps-reveal-3"
                        style="{{ $isMonk
                        ...
                        <div class="space-y-3">
                            @if ($person->phone)
                                <a href="tel:{{ $person->phone }}"
                                    ...
                                    <div>
                                        <p style="font-size:9px; color:rgba(124,77,15,0.42); font-weight:700; letter-spacing:0.1em; text-transform:uppercase; margin-bottom:1px;">ໂທລະສັບ</p>
                            ...
                            @if ($person->email)
                                <a href="mailto:{{ $person->email }}"
                                    ...
                                    <div class="min-w-0">
                                        <p style="font-size:9px; color:rgba(124,77,15,0.42); font-weight:700; letter-spacing:0.1em; text-transform:uppercase; margin-bottom:1px;">ອີເມວ</p>
                            ...
                            @if ($person->facebook)
                                <a href="{{ $person->facebook }}" target="_blank" rel="noopener noreferrer"
                                    ...
                                    <div>
                                        <p style="font-size:9px; color:rgba(124,77,15,0.42); font-weight:700; letter-spacing:0.1em; text-transform:uppercase; margin-bottom:1px;">ໂຊຊຽວ</p>
                ...
                {{-- Department card --}}
                @if ($person->department)
                    <div class="rounded-2xl p-4 ps-reveal ps-reveal-4" style="background:white; border:1px solid rgba(0,0,0,0.07);">
                        <p class="font-bold uppercase tracking-widest mb-3"
    ...
    <section style="background:#F5F0E8; border-top:1px solid rgba(200,149,58,0.12);">
        <div class="max-w-5xl mx-auto px-4 sm:px-8 py-12">
            ...
            <div class="flex items-center gap-3 mb-8">
                <div class="h-px flex-1" style="background:linear-gradient(to right, transparent, rgba(200,149,58,0.3));"></div>
                ...
                <div class="h-px flex-1" style="background:linear-gradient(to left, transparent, rgba(200,149,58,0.3));"></div>
            </div>
